...
const fuzzyFilter = <T, K extends keyof T>(
    items: T[], 
    term: string,
    key: K
): T[] => {
    if (items.length === 0 || !term) return [];
    ...
    return items.filter(item => {
        ...
        if (itemTextLower.includes(normalizedSearchTerm)) return true;
        
        const itemWords = itemTextLower.split(/\s+/).filter(Boolean);
        return searchWords.every(searchWord => {
            ...
            return itemWords.some(itemWord => 
                ...
                levenshteinDistance(searchWord, itemWord) <= maxDistance
            );
        });
    });
};
...
const App: React.FC = () => {
  ...
  const handleOrgSearch = useCallback(async (term: string): Promise<SearchResults> => {
      const orgsByCarrierName = fuzzyFilter(allOrganisations, term, 'name');
  ...
  const handleTitleSearch = useCallback(async (term: string): Promise<SearchResults> => {
    const projectsByTitle = fuzzyFilter(allProjects, term, 'title');
    const eventsByTitle = fuzzyFilter(allFundraisingEvents, term, 'title');

    return {
        organisations: [], // This search doesn't return organisations directly
        projects: projectsByTitle,
        fundraisingEvents: await processFundraisingEvents(eventsByTitle),
    };
  }, [allProjects, allFundraisingEvents, processFundraisingEvents]);
  ...
  const handleSearch = useCallback(async (terms: SearchTerms) => {
    ...
    const activeTerm = terms.idTerm || terms.contactTerm || terms.orgTerm || terms.titleTerm;
    ...
    try {
        ...
        } else if (terms.titleTerm) {
            finalResults = await handleTitleSearch(terms.titleTerm);
  ...
  }, [handleIdSearch, handleContactSearch, handleOrgSearch, handleTitleSearch]);
        ...
        <header className="text-center mb-8">
          <div className="flex justify-center items-center gap-3">
            ...
            <button
              ...
              aria-label="Informationen zum Tool anzeigen"
            >
